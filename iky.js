...
let ayang = fs.readFileSync('./media/kiki.jpg')
               buttons =  [{buttonId: `${prefix}allmenu`, buttonText: {displayText: '📜ALL MENU'}, type: 1},{buttonId: `${prefix}rules`, buttonText: {displayText: '✖RULES'}, type: 1},{buttonId: `${prefix}owner`, buttonText: {displayText: '👥CREATOR'}, type: 1},]
                    ...
                    dimas.sendMessage(from, { contentText: menu, footerText: '_*Jika kalian pakai wa mod,bisa mengetik !command*_\n\n@Dimas Edr', buttons: buttons, headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: ayang, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage', { quoted: dim})
   break
    case 'allmenu':
              if (!isRegistered) return sendButMessage(from, maaf, mod, tombar,{quoted:dim})
       groups = dimas.chats.array.filter(v => v.jid.endsWith('g.us'))
...
`
                   sendButMessage(from, wibu, `@Dimas Edr`, [
          {
            buttonId: `!menu`,
            buttonText: {
...
dimas.sendMessage(_.jid, 
   ...
   "buttonText": {"displayText": "🔖VERIFIKASI"
             ...
             for (let _ of anu) { 
             dimas.sendMessage(_.jid, { contentText: `*🌹 SIARAN IZUMI BOTZ 🌹*\n\n${body.slice(4)}`, footerText: '©Created by Dimas EDR', buttons: [{ buttonId: `${prefix}menu`, buttonText: { displayText: 'MENU' }, type: 1 }, { buttonId: `${prefix}owner`, buttonText: { displayText: 'OWNER' }, type: 1 }], headerType: 6, locationMessage: { degreesLatitude: 0, degreesLongitude: 0, jpegThumbnail: bc2 }}, 'buttonsMessage') 
} 
             reply('Suksess broadcast') 
} 
             break
